...
The `Checks` workflow always builds and tests. Automatic deployment is deliberately off until the first manual release is verified. To enable it, configure the `vessel-production` GitHub environment, add repository variable `VESSEL_DEPLOY_ENABLED=true`, and set repository secrets `VESSEL_DEPLOY_SSH_KEY` (a dedicated SSH deploy key with access as `ubuntu`) and `VESSEL_DEPLOY_KNOWN_HOSTS` (the verified server host key line, obtained and checked out of band). The `ubuntu` account needs passwordless sudo for `/usr/local/sbin/vessel-deploy` only, and permission to stage into its own `~/vessel-staging` directory. A push to `main` then deploys the tested artifact. Never use the account owner password or companion key as a CI secret.
