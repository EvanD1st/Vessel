...
The command prints only the path to a mode-0600 login file. Read the temporary password over your own SSH session with `sudo cat <printed-path>`, sign in at the HTTPS site, and change the password immediately. Do not paste the password or the login file into issues, chat, or the repository. Public registration and password reset are disabled for this invite-only release; the owner manages accounts from the dashboard.

## GitHub Actions deployment

The `Checks` workflow always builds and tests. Automatic deployment is deliberately off until the first manual release is verified. To enable it, configure the `vessel-production` GitHub environment, add repository variable `VESSEL_DEPLOY_ENABLED=true`, and set repository secrets `VESSEL_DEPLOY_SSH_KEY` (a dedicated SSH deploy key with access as `ubuntu`) and `VESSEL_DEPLOY_KNOWN_HOSTS` (the verified server host key line, obtained and checked out of band). The `ubuntu` account needs passwordless sudo to install the reviewed deploy helper and run `/usr/local/sbin/vessel-deploy`, plus permission to stage files in its own `~/vessel-staging` directory. A push to `main` then deploys the tested artifact. Never use the account owner password or companion key as a CI secret.
